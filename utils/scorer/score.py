...
def _read_gold_and_pred(gold_fpath, pred_fpath):
    ...
    with open(gold_fpath) as gold_f:
        for line_res in gold_f:
            line_number, label = line_res.strip().split('\t')  # process the line from the res file
            gold_labels[int(line_number)] = round(float(label))
...
def print_metrics_info(line_separator):
    ...
    logging.info(
        'Average Precision is the precision@N, estimated only @ each relevant line_number and then averaged over the number of relevant line_numbers.')
    logging.info(
        'Reciprocal Rank is the reciprocal of the rank of the first relevant line_number in the list of predictions sorted by score (descendingly).')
